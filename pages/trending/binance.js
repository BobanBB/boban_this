...
export default function Home() {
    ...
    const fetchAllCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/trending/binance?page=${pageAll}&limit=10`);
    ...
    const fetchPromotedCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/trending/binance?page=${pagePromoted}&limit=10&promoted=true`);
            const data = await response.json();
    ...
    return (
        <>
            <Head>
                <title>Binance</title>
                <meta name="description" content="Binance - Top trending Crypto Coins today | bobantoken.com" />
