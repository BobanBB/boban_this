...
export default function Home() {
    ...
    const fetchAllCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/new/tron?page=${pageAll}&limit=10`);
    ...
    const fetchPromotedCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/new/tron?page=${pagePromoted}&limit=10&promoted=true`);
            const data = await response.json();
    ...
    return (
        <>
            <Head>
                <title>Tron</title>
                <meta name="description" content="Tron - Top trending Crypto Coins today | bobantoken.com" />
