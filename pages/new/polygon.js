...
export default function Home() {
    ...
    const fetchAllCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/new/polygon?page=${pageAll}&limit=10`);
    ...
    const fetchPromotedCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/new/polygon?page=${pagePromoted}&limit=10&promoted=true`);
            const data = await response.json();
    ...
    return (
        <>
            <Head>
                <title>Polygon</title>
                <meta name="description" content="Polygon - Top trending Crypto Coins today | bobantoken.com" />
