...
export default function Home() {
    ...
    const fetchAllCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/gainers?page=${pageAll}&limit=10`);
    ...
    const fetchPromotedCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/gainers?page=${pagePromoted}&limit=10&promoted=true`);
            const data = await response.json();
    ...
    return (
        <>
            <Head>
                <title>Daily Gainers</title>
                <meta name="description" content="Daily Gainers - Top trending Crypto Coins today | bobantoken.com" />
