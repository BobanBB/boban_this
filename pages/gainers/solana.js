...
export default function Home() {
    ...
    const fetchAllCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/gainers/solana?page=${pageAll}&limit=10`);
    ...
    const fetchPromotedCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/gainers/solana?page=${pagePromoted}&limit=10&promoted=true`);
            const data = await response.json();
    ...
    return (
        <>
            <Head>
                <title>Solana</title>
                <meta name="description" content="Solana - Top trending Crypto Coins today | bobantoken.com" />
