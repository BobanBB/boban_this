...
export default function Home() {
    ...
    const [cryptoStats, setCryptoStats] = useState([]); // All trending coins
    ...
    // Fetch all trending coins
    const fetchAllCoins = useCallback(async () => {
        setIsFetchingAll(true);
        try {
            const response = await fetch(`/api/trending?page=${pageAll}&limit=10`);
    ...
    const fetchPromotedCoins = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/trending?page=${pagePromoted}&limit=10&promoted=true`);
            const data = await response.json();
